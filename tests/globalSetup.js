...
module.exports = async function (globalConfig, projectConfig) {
  global.setEnvironmentValue = (key, value) => {
    ...
    if (myVarLine == -1)
      envLines[envLines.length] = `${key}=${value}`;
    else // update the value of the variable
      envLines[myVarLine] = `${key}=${value}`;
  ...
  global.appConfig = {
    name: appName,
    id: `com.metrological.app.${appName.replace(/[^A-Z0-9]/gi, '')}`,
    appPath: process.cwd() + `/com.metrological.app.${appName.replace(/[^A-Z0-9]/gi, '')}`
  }
